...
//       }
//       updatedFields.companiesLogo = `uploads/${req.files['companiesLogo'][0].filename}`;
//     }
...
const updateCourierRate = async (req, res) => {
  try {
    const updatedFields = { ...req.body };  // Extract the fields from the request body.

    const rate = await CourierRate.findById(req.params.id);  // Find the existing rate by its ID.
    if (!rate) {
      return res.status(404).json({ message: 'Courier rate not found.' });
    }
    ...
    const updatedRate = await CourierRate.findByIdAndUpdate(req.params.id, updatedFields, { new: true });
    res.status(200).json(updatedRate);  // Return the updated rate in the response.
  } catch (error) {
    console.error('Error updating courier rate:', error);
    res.status(500).json({ message: 'An error occurred while updating the courier rate.' });
  }
...
const uploadCourierRatesCSV = async (req, res) => {
  try {
    ...
    const filePath = file.path; // ✅ file.path will be absolute path from multer
    if (!filePath) {
